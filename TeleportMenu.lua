...
local tpTable = {
	...
	{id = 61447, type = "spell"}, -- Astral Recall (Shaman)
	...
	--more tps
	{id = 312372, type = "spell"}, --Return to camp
	
	-- Misc
	{id = 460905, type = "spell"}, -- Warband Bank
	{id = 206268, type = "toy"}, -- Transmog Toy
	{id = 156833, type = "toy"}, --Katys Stampwhistle
	{id = 61447, type = "mount"}, -- Travellers Tundra mammot
...
function tpm:setToolTip(self, type, id, hs)
	...
	elseif type == "mount" then
		GameTooltip:SetMountBySpellID(id)
...
local function createAnchors()
	...
	for i, teleport in ipairs(tpTable) do
		...
		elseif teleport.type == "spell" and IsSpellKnown(teleport.id) then
			...
			known = true
		elseif teleport.type == "mount" then
			local _, _, _, _, _, _, _ , _, _,_, collected = C_MountJournal.GetMountInfoByID(C_MountJournal.GetMountFromSpell(teleport.id))
			mountname,_, texture = C_MountJournal.GetMountInfoByID(C_MountJournal.GetMountFromSpell(teleport.id))
			known = true
		end
		...
		-- Create Stuff
		if known and (teleport.type == "toy" or teleport.type == "item" or teleport.type == "spell" or teleport.type == "mount") then
